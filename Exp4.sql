...
-- 🔴 MySQL detects deadlock, aborts one transaction:
...
-- 1️⃣ Without MVCC (traditional locking using SELECT FOR UPDATE)
-- Session A
START TRANSACTION;
...
-- 2️⃣ With MVCC (normal SELECT in REPEATABLE READ isolation)
-- Session A
SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ;
START TRANSACTION;
SELECT * FROM StudentEnrollments WHERE student_id = 1;
...
SELECT * FROM StudentEnrollments WHERE student_id = 1;
-- ✅ Still sees old value (2024-07-10)

-- Session A after commit
COMMIT;
SELECT * FROM StudentEnrollments WHERE student_id = 1;
-- ✅ Sees updated value (2024-08-20)
...
-- ✅ Part A Demo: Successful inserts (no duplicates)
START TRANSACTION;
...
-- ✅ Part B Demo: Row Locking with SELECT FOR UPDATE
...
-- ✅ Part C Demo: Consistency with locking
-- User A
START TRANSACTION;
UPDATE StudentEnrollments
